...
export const create = asyncHandler(async (req, res) => {
  try {
    ...
    const result = await service.create(ownerData, photo);
...
export const update = asyncHandler(async (req, res) => {
  try {
    ...
    const result = await service.update(req.params.email, ownerData, photo);
    res.status(200).json(result);
  } catch (error) {
    const status = error.status || 500;
    res.status(status).json({ message: error.message });
...
export const elevate = asyncHandler(async (req, res) => {
  try {
    ...
    const result = await service.elevate(req.params.email, ownerData, photo);
    res.status(201).json(result);
  } catch (error) {
    const status = error.status || 500;
    res.status(status).json({ message: error.message });
